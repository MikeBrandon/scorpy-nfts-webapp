...
import ScorpyNFT from "$lib/utils/scorpyNFT.json";
	...
	async function checkIfWalletIsConnected() {
		...
		if (accounts.length !== 0) {
            ...
            askContractForTotalNumberOfNFTs();
		} else {
        ...
        try {
            ...
            if (ethereum) {
                ...
                connectedContract.on('TotalNFTs', (from, TotalNFTs) => {
                    ...
                    console.log('TotalNFTs: ', mintedNumber);
                })
            } else {
    ...
    async function askContractForTotalNumberOfNFTs() {
        try {
            const { ethereum } = window;
    ...
    <div class="App">
        <div class="container">
          <div class="header-container">
			...
			{:else}
				<button on:click={askContractToMintNft} class="cta-button connect-wallet-button" disabled={mintedNumber>=30}>
					Mint NFT
				</button>
            {/if}
          </div>
          ...
          <p class="sub-text">
            TotalNFTs Mined: {(mintedNumber!=0) ? mintedNumber - 1 : mintedNumber}/30
          </p>
